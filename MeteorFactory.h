...
public:
  ...
  virtual bool operation() override {
    // ≈сли нужен новый метеорит - создать его
    if (waitingNew) {
    ...
    // ≈сли создан новый метеорит, нужно запустить его
    if (createdNew && countOfMeteors < 3) {
      Meteor *meteor = &meteors[countOfMeteors];
      meteor->speed = 8;
      // ≈сли долетел до земли - станим всех вокруг
      if (!meteor->operation()) {
        int left = GetX() - GetRange();
    ...
    // “ем временем, нужно каждый упавший метеорит пускаааать по земле
    for (int i = 0; i < 3; i++) {
    ...
    // —килл заканчиваетс€, когда все метеориты запущены, и все метеориты пролетели уже
